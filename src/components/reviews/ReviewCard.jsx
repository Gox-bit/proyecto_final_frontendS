...
const ReviewCard = ({ review, currentUserId, currentUserRole, onUpdate, onDelete }) => {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-4 shadow-lg">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {isAdmin && isOwner && <span className="text-[10px] bg-red-500 text-white px-1 rounded ml-2">ADMIN</span>}
        ...
        <div className="flex items-center gap-1 bg-gray-950 px-3 py-1 rounded-full border border-gray-800">
           {isEditing ? (
             <select value={editRating} onChange={(e) => setEditRating(Number(e.target.value))} className="bg-transparent text-yellow-400 font-bold text-sm">
      ...
      <div className="mb-4">
        {isEditing ? (
          <textarea value={editComment} onChange={(e) => setEditComment(e.target.value)} className="w-full bg-gray-950 border border-gray-700 rounded p-3 text-gray-300" />
        ) : (
          <p className="text-gray-300 text-sm whitespace-pre-wrap">{review.comentario}</p>
        )}
      </div>
      ...
      {(canEdit || canDelete) && (
        <div className="flex justify-end gap-2 pt-4 border-t border-gray-800">
          {isEditing ? (
            <>
               <button onClick={() => setIsEditing(false)} className="text-gray-400 hover:text-white px-3 py-1"><X className="h-4 w-4"/></button>
               <button onClick={handleSave} className="text-purple-400 hover:text-purple-300 px-3 py-1"><Save className="h-4 w-4"/></button>
            </>
          ) : (
            <>
